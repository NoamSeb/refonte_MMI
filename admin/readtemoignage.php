...
$resulttemoignage = detailTemoignage($_GET["id_temoignage"]);
session_start();
        ...
        <div class="spacebetween">
            ...
            <?php
                    foreach ($resulttemoignage as $value) {
                        echo '<div><a type="button" class="btn btn-warning" href="edittemoignage.php?action=edit & id_temoignage=' . $value['id_temoignage'] . '">&#9999; MODIFIER</a>&nbsp;&nbsp;<a type="button" class="btn btn-danger" onclick="return confirm(`Êtes-vous sûr de vouloir supprimer l\'article ?`)" href="deletetemoignage.php?type=id_temoignage&delete & id_temoignage=' . $value['id_temoignage'] . '">&#128465; SUPPRIMER</a></div>';
                    };
            ?>
        </div>
            <?php
            foreach ($resulttemoignage as $value) {
                echo
                '<article><h2>' . $value["nom_etudiant"] . '</h2>' .
                    '<h6>' . $value["contenu_temoignage"] . '</h6>' .
                    '<p>' . $value["promo_etudiant"] . '</p></article>';
            }
            ?>
